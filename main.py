...
def run(name):
    ...
    summary = (
        data.groupby(["epsilon", "variant"])
        .agg(
            train_T=("train_T", "mean"),
            train_std=("train_T", "std"),
            test_T=("test_T", "mean"),
            test_std=("test_T", "std"),
...
def plot(name, data, out):
    ...
    x = data[
        (data.epsilon == min(EPSILONS)) & (data.variant != "original")
    ]
